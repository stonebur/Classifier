...
class FirstWindow(QWidget):
	...
	def ClickMale(self):
		...
		#called at the very end to display
		#Picture by itself
class FinalViewWindow(QWidget):

	def __init__(self, picname):
		super().__init__()
		self.title = 'Your Perfect Match'
		self.left = 10
		self.top = 10
		self.width = 640
		self.height = 480
	...
	def initUI(self):
		...
		self.pic.setPixmap(QPixmap(self.picname))	
		self.pic.show()								
...
class Window(QWidget):
	...
	def on_clickYes(self):
		...
		try:                                #if it does not hit 200 retrain anyway
	...
	def trainAndSelect(self):
		if (self.yesCounter >= 200 and self.noCounter >= 200) or self.override==1:
				self.pics = {}
				counter = 0
				os.chdir("../../../")
				os.system("./train.sh")
				...
				self.ex2=FinalViewWindow(perfMatch)
				self.ex2.show()
				self.hide()					#may want to be comment out self.hide
											#as it appears that it prepares for a second run
				self.yesCounter = 180
				self.noCounter = 180
